...
var selectAll: string;    //declarando variável string

begin
  selectAll := 'SELECT * FROM membros WHERE 1=1';  //armazenando um trecho de código sql como string na variável

  if Trim(edtCampoPesquisa.Text) <> '' then
  begin
    //variável selectAll recebe ela mesma + um trecho de código sql e o valor do campo edtCampoPesquisa entre aspas
    selectAll := selectAll + ' AND nome LIKE ' + QuotedStr('%' + Trim(edtCampoPesquisa.Text) + '%');
  end;


  cdsMembros.Close; //fecha o client dataset
  cdsMembros.CommandText := selectAll; //seta no CommandText a variável selectAll com os valores que ela recebeu
  cdsMembros.Open; //abre o client dataset

  //se o clientDataset for vazio
  if cdsMembros.IsEmpty then
  begin
    ShowMessage('Não há registro na consulta');
    exit;
  end;
